...
StatsDClient.prototype.gauge = function gauge(name, value, tags) {
    this._socket.send(this.options.prefix + name + this.formatTags(tags) +":" + value + "|g" );
...
StatsDClient.prototype.gaugeDelta = function gaugeDelta(name, delta, tags) {
    var sign = delta >= 0 ? "+" : "-";
    this._socket.send(this.options.prefix + name + this.formatTags(tags) + ":" + sign + Math.abs(delta) + "|g" );
...
StatsDClient.prototype.set = function set(name, value, tags) {
    this._socket.send(this.options.prefix + name + this.formatTags(tags) + ":" + value + "|s" );
...
StatsDClient.prototype.counter = function counter(name, delta, tags) {
    this._socket.send(this.options.prefix + name + this.formatTags(tags) + ":" + delta + "|c");
...
StatsDClient.prototype.timing = function timing(name, time, tags) {
    ...
    this._socket.send(this.options.prefix + name +this.formatTags(tags) + ":" + t + "|ms"  );
...
StatsDClient.prototype.histogram = function histogram(name, value, tags) {
    this._socket.send(this.options.prefix + name +this.formatTags(tags) + ":" + value + "|h"  );
